...
from module17.app.models import Task, User
from module17.app.schemas import CreateTask, UpdateTask, CreateUser
...
async def create_task(db: Annotated[Session, Depends(get_db)], create_task: CreateTask, user_id: int):
    ...
    if user is None:
        ...
        db.execute(
            insert(Task).values(title=create_task.title, content=create_task.content, priority=create_task.priority,
                                slug=slugify(create_task.title), user_id=user_id))
        db.commit()
